...
% As above, 'features_pos' is N by D matrix where N is the number of faces and D
% is the template dimensionality, which would be
%   (feature_params.template_size / feature_params.hog_cell_size)^2 * 31
% if you're using the default vl_hog parameters
D = (feature_params.template_size / feature_params.hog_cell_size)^2 * 31;
features_pos = zeros(num_images, D); % initailize

for i = 1:num_images
    img = im2single(imread(strcat(train_path_pos, '/', image_files(i).name)));
    % API : hog = vl_hog(image, hog_cellsize)
    features_pos(i ,:) = reshape(vl_hog(img, feature_params.hog_cell_size), 1, D);
    %features_pos(i ,:) = My_HoG(img, feature_params.hog_cell_size);
end
